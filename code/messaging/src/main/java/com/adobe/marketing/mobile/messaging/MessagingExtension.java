...
public final class MessagingExtension extends Extension {
    ...
    private void addXDMData(final Map<String, Object> eventData, final Map<String, Object> xdmMap) {
        ...
        try {
            ...
            // Add decisioning data if not already present
            addDecisioningDataIfMissing(xdmMap, xdmMapObject);
    ...
    /**
     * Adds or ensures decisioning data structure in the XDM map with all required fields.
     *
     * @param xdmMap the XDM map to be updated
     * @param xdmMapObject the original XDM data map from the push payload
     */
    private void addDecisioningDataIfMissing(
            final Map<String, Object> xdmMap, final Map<String, Object> xdmMapObject) {
        ...
        if (xdmMap.containsKey(MessagingConstants.TrackingKeys.EXPERIENCE)
                && xdmMap.get(MessagingConstants.TrackingKeys.EXPERIENCE) instanceof Map) {
            experience = (Map<String, Object>) xdmMap.get(MessagingConstants.TrackingKeys.EXPERIENCE);
        }
        ...
        if (experience.containsKey(MessagingConstants.TrackingKeys.DECISIONING)
            ...
            Log.debug(
                    MessagingConstants.LOG_TAG,
                    SELF_TAG,
                    "Decisioning object exists, ensuring all required fields are present.");
        }

        // Try to extract decisioning data from the original push payload
        Map<String, Object> payloadDecisioningData = extractDecisioningFromPayload(xdmMapObject);
        
        if (decisioning == null) {
            // No existing decisioning, use payload data or create default
            if (payloadDecisioningData != null) {
                decisioning = payloadDecisioningData;
            } else {
                decisioning = new HashMap<>();
            }
        }

        // Ensure all required fields are present in decisioning
        ensureDecisioningFields(decisioning);

        // Update the decisioning in experience
        experience.put(MessagingConstants.TrackingKeys.DECISIONING, decisioning);
        Log.debug(
                MessagingConstants.LOG_TAG,
                SELF_TAG,
                "Successfully ensured decisioning data structure in XDM tracking map.");
    }
     ...
     */
    private void ensureDecisioningFields(final Map<String, Object> decisioning) {
     ...
     */
    private void ensurePropositionFields(final Map<String, Object> proposition) {
        // Ensure id exists
     ...
     */
    private void ensureItemFields(final Map<String, Object> item) {
        // Ensure id exists
     ...
     */
    private Map<String, Object> createDefaultProposition() {
     ...
     */
    private Map<String, Object> createDefaultItem() {
    ...
    /**
     * Extracts decisioning information from the push payload XDM data.
     *
     * @param xdmMapObject the XDM map object from the push payload
     * @return a Map containing decisioning data, or null if not found
     */
    private Map<String, Object> extractDecisioningFromPayload(
            final Map<String, Object> xdmMapObject) {
        try {
            // Look for decisioning data in the payload
            // Could be in _experience.decisioning or directly in the payload
            if (xdmMapObject.containsKey(MessagingConstants.TrackingKeys.EXPERIENCE)) {
                Object expObj = xdmMapObject.get(MessagingConstants.TrackingKeys.EXPERIENCE);
                if (expObj instanceof Map) {
                    Map<String, Object> experience = (Map<String, Object>) expObj;
                    if (experience.containsKey(MessagingConstants.TrackingKeys.DECISIONING)) {
                        Object decisioning = experience.get(MessagingConstants.TrackingKeys.DECISIONING);
                        if (decisioning instanceof Map) {
                            return (Map<String, Object>) decisioning;
                        }
                    }
                }
            }

            // Check in cjm or mixins
            Map<String, Object> mixins = null;
            if (xdmMapObject.containsKey(MessagingConstants.TrackingKeys.CJM)) {
                Object cjmObj = xdmMapObject.get(MessagingConstants.TrackingKeys.CJM);
                if (cjmObj instanceof Map) {
                    mixins = (Map<String, Object>) cjmObj;
                }
            } else if (xdmMapObject.containsKey(MessagingConstants.TrackingKeys.MIXINS)) {
                Object mixinsObj = xdmMapObject.get(MessagingConstants.TrackingKeys.MIXINS);
                if (mixinsObj instanceof Map) {
                    mixins = (Map<String, Object>) mixinsObj;
                }
            }

            if (mixins != null && mixins.containsKey(MessagingConstants.TrackingKeys.EXPERIENCE)) {
            ...
            // If no decisioning data found, return null
            return null;
        } catch (ClassCastException e) {
            Log.warning(
                    MessagingConstants.LOG_TAG,
                    SELF_TAG,
                    "Failed to extract decisioning data from push payload: %s",
                    e.getMessage());
            return null;
        }
    }
